...
parser.add_argument("-plr", "--patience_lr", type=int, default=500, help="factor of learning rate when it is reduced")
parser.add_argument("-pes", "--patience_es", type=int, default=5000, help="early stopping")
...
if __name__ == "__main__":
    ...
    os.makedirs(f"./weights_EMIDEC_{args.task}/", exist_ok=True)
    ...
    if args.type_train == "train_test":
        ...
        with open("csv_files/EMIDEC_val_train_full.csv", mode="r") as f:
            reader = csv.DictReader(f)
            list_test_subject = [row["path"] for row in reader]
    else:
    ...
    print("Type train:", args.type_train)
    ...
    if checkpoint_paths and cfg.TRAIN.LOAD_CHECKPOINT:
        ...
        segmenter = Segmenter_EMIDEC.load_from_checkpoint(
            ...
            batch_size_predict=args.batch_size,
        )
